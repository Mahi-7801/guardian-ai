                ...
                <main className="flex-1 p-4 sm:p-6 overflow-y-auto scrollbar-thin">
                    <div className="max-w-6xl mx-auto space-y-8">
                        <div className="flex items-center gap-4 border-b border-border pb-6">
                            ...
                            <div>
                                <h1 className="text-3xl font-bold">Guardian AI Neural Taxonomy</h1>
                            ...
                            <div className="lg:col-span-3 space-y-6">
                                {taxonomy && taxonomy[activeSection] ? (
                                    <div className="animate-fade-in space-y-8">
                                        <div className="glass-card p-8 bg-gradient-to-br from-card to-background border-primary/20 relative overflow-hidden">
                                        ...
                                        <div className="glass-card p-6 bg-secondary/20 border-border/50">
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                {activity && (
                                                    <div className="p-4 rounded-xl bg-primary/5 border border-primary/20 space-y-2 animate-in fade-in slide-in-from-bottom-2">
                                                        <div className="flex justify-between items-center">
                                                            <span className="text-[9px] font-black uppercase text-primary/70 tracking-widest">Active Process ({activeSection.replace(/_/g, " ")})</span>
                                                            <span className="text-[9px] font-mono text-muted-foreground">{new Date(activity.timestamp).toLocaleTimeString()}</span>
                                                        </div>
                                                ...
                                                <div className="flex items-center gap-2 mt-2 p-3 bg-primary/5 rounded-lg border border-primary/10">
                                                    ...
                                                    <span className="text-[10px] font-bold text-primary uppercase">Neural Logic: Fully Operational across all {activeSection.replace(/_/g, " ")} vectors</span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        {activeSection === 'crosint' && signals.length > 0 && (
                                            <div className="glass-card p-6 border-primary/30 bg-primary/5 animate-fade-in">
                                                ...
                                                <div className="space-y-3">
                                                    {signals.map((s, i) => (
                                                        <div key={i} className="flex justify-between items-center p-3 rounded-lg bg-background/50 border border-border/50 group hover:border-primary/30 transition-all">
                                                            <div className="flex items-center gap-3">
                                                                <div className={`w-1.5 h-1.5 rounded-full ${s.verified ? 'bg-success' : 'bg-warning animate-pulse'}`} />
                                                                <div>
                                                                    <p className="text-xs font-bold">{s.event}</p>
                                                                    <p className="text-[9px] text-muted-foreground uppercase">{s.location} • {s.impact} Impact</p>
                                                                </div>
                                                            </div>
                                                            <span className="text-[8px] font-mono p-1 bg-secondary rounded text-muted-foreground">ID: {s.id}</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
